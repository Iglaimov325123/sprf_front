          ...
          <div class="grid devision flex flex-just-b">
            ...
            <aside class="col-4">
              <template v-for="link in page.LINKS">
                <card-simple :link="link.CODE" :desc="link.NAME"></card-simple>
              </template>
            </aside>
          </div>
        </div>
...
export default {
  async asyncData (context) {
    let page = await context.app.$axios.get('/misc/simplePage/activity')
  ...
  mounted() {
    ...
    // that.$axios.get('/misc/simplePage/activity')
