  ...
  <div class="page-content" data-type="inner">
    <section class="blue with-expand" data-type="inner-head">
      <div class="head-main center flex flex-just-b flex-center-v">
        <h1>{{$store.getters.getLang == 'en' ? 'Expertise of draft laws' : 'Экспертиза законопроектов'}}</h1>
    ...
    <filters ref="filter" filer="items" :hidepersons="true" post="/examination/list" :archive="isArchive"></filters>
    ...
    <section data-type="page-content">
      <div class="center">
        ...
        <div class="page-contain">
          <div class="tag-items">
            <template v-for="check in items">
              <item-tags
              :name="check.NAME"
              :date="check.DATE_CREATE"
              :link="'/expertise/'+check.CODE"
              :tags="check.TAGS"
              v-if="!check.IS_ACCORDION"
              ></item-tags>
          ...
          <a :href="'/expertise/page/'+page" class="btn full-width big" @click.prevent="loadItems" v-if="page != totalPage && items.length > 0">{{$store.getters.getLang == 'en' ? 'Load more' : 'Загрузить предыдущие материалы'}}</a>
...
export default {
  async asyncData (context) {
    let info = await context.app.$axios.get(`/examination/list`, {
      params: {
        count: 5,
        page: 1
      }
  ...
  mounted() {
    ...
    // that.$axios.get(`/examination/list`, {
    //   params: {
    //     count: that.count,
    //     page: that.page
    //   }
