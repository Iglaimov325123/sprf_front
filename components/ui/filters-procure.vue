      ...
      <div class="filter-inner flex flex-wrap" style="height: 0">
        ...
        <div class="filter-inner-content">
          <div class="filter-top flex col-12">
            <div class="filter-col col-4">
              <div class="filter-name" @click="openFilterCol">
                {{$store.getters.getLang == 'en' ? 'Type' : 'Тип закупки'}}
              ...
              <div class="filter-col-inner">
                ...
                <vue-simplebar class="filter-value">
                  <ul class="select">
                    <li :class="type.selected ? 'selected' : ''" v-for="type in types" @click="toggleType(type)">{{type.name}}</li>
                  </ul>
            ...
            <div class="filter-col col-4">
              <div class="filter-name" @click="openFilterCol">
                {{$store.getters.getLang == 'en' ? 'Status' : 'Состояние закупки'}}
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24.89 8.01">
                  <path d="M13,8.12.55,1.89,1.45.11,13,5.88,24.55.11l.9,1.78Z" transform="translate(-0.55 -0.11)" fill="#04092a"/>
                </svg>
              </div>
              <div class="filter-col-inner">
                ...
                <vue-simplebar class="filter-value">
                  <ul class="select">
                    <li :class="type.selected ? 'selected' : ''" v-for="type in status" @click="toggleType(type)">{{type.name}}</li>
                  </ul>
                  <div class="real-select">
                    <select name="type[]">
                      <option :value="type.value" v-for="type in status" :selected="type.selected">{{type.name}}</option>
                    </select>
                  </div>
                </vue-simplebar>
              </div>
...
export default {
  data() {
    return {
      ...
      types: [
        {name: 'Запрос котировок', value: 8732, selected: false},
        {name: 'Открытый конкурс', value: 8733, selected: false},
        {name: 'Электронный конкурс', value: 8730, selected: false},
        {name: 'Закупка у единственного поставщика', value: 8731, selected: false}
      ],

      status: [
        {name: 'Открытая', value: 'active', selected: false},
        {name: 'Прием документов завершен', value: 'stoped', selected: false},
        {name: 'Завершена', value: 'finished', selected: false}
      ],
  ...
  methods: {
    ...
    getFilterObject() {
      ...
      for(let i = 0; i < that.status.length; i++) {
        if(that.status[i].selected) {
          this.statusFilter.push(that.status[i].value);
        }
      }
      ...
      if(this.typesFilter.length != 0 || this.statusFilter.length != 0 || ((startDate !== '01.01.1990' || endDate !== '01.01.3000') && this.date)) {
      ...
      return {
        ...
        progress: this.statusFilter.join(),
    ...
    filterItems() {
      ...
      if((startDate != '01.01.1990' && endDate != '01.01.3000') || this.typesFilter.length != 0 || this.statusFilter.length != 0) {
        this.$parent.hasFilter = true;
      } else {
        this.$parent.hasFilter = false;
      }
      ...
      that.$axios.get(that.post, {
        ...
        .then(function (response) {
          ...
          if(that.$parent.totalPage == 1) {
            that.$parent.isLastPage = true;
          } else {
    ...
    resetFilter() {
      ...
      console.log(that.$parent.hasFilter);
