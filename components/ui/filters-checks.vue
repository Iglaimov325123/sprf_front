      ...
      <div class="filter-inner flex flex-wrap" style="height: 0">
        ...
        <div class="filter-inner-content">
          <div class="filter-top flex col-12">
            <div class="col-3 filter-col" v-if="!hidepersons">
              <div class="filter-name" @click="openFilterCol">
                {{$store.getters.getLang == 'en' ? 'Industry' : 'Отрасль'}}
              ...
              <div class="filter-col-inner">
                ...
                <vue-simplebar class="filter-value">
                  <ul class="select">
                    <li :class="type.selected ? 'selected' : ''" v-for="type in scopes" @click="toggleType(type)">{{type.VALUE}}</li>
                  </ul>
                  <div class="real-select">
                    <select name="type[]">
                      <option :value="type.ID" v-for="type in scopes" :selected="type.selected">{{type.VALUE}}</option>
                    </select>
                  </div>
                </vue-simplebar>
              </div>
            ...
            <div class="col-3 filter-col" v-if="!hidepersons">
              <div class="filter-name" @click="openFilterCol">
                {{$store.getters.getLang == 'en' ? 'Region' : 'Регион'}}
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24.89 8.01">
                  <path d="M13,8.12.55,1.89,1.45.11,13,5.88,24.55.11l.9,1.78Z" transform="translate(-0.55 -0.11)" fill="#04092a"/>
                </svg>
              </div>
              <div class="filter-col-inner">
                ...
                <vue-simplebar class="filter-value">
                  <ul class="select">
                    <li :class="format.selected ? 'selected' : ''" v-for="format in regions" @click="toggleType(format)">{{format.VALUE}}</li>
                  </ul>
                  <div class="real-select">
                    <select name="regions[]" multiple>
                      <option :value="format.ID" v-for="format in regions" :selected="format.selected">{{format.VALUE}}</option>
                    </select>
                  </div>
                </vue-simplebar>
              </div>
            </div>
            <div class="col-4 filter-col" v-if="statements">
              <div class="filter-name" @click="openFilterCol">
                Выпуск бюллетеня
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24.89 8.01">
                  <path d="M13,8.12.55,1.89,1.45.11,13,5.88,24.55.11l.9,1.78Z" transform="translate(-0.55 -0.11)" fill="#04092a"/>
            ...
            <div class="filter-col" :class="hidepersons ? 'col-4' : 'col-3'">
              <div class="filter-name" @click="openFilterCol">
...
export default {
  ...
  mounted() {
      ...
      that.$axios.get('/bulletin/category')
        .then(function (response) {
